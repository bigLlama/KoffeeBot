...
class fun(commands.Cog):
    def __init__(self, client):
        ...
        @client.tree.command(name="meme", description="Enjoy some quality memes!")
        ...
        @client.tree.command(name="roast", description="Roast someone")  # Roast (Add more roasts in future)
        @app_commands.describe(person="The person you wish to roast")
        async def roast(interaction: discord.Interaction, person: discord.Member = None):
        ...
        @client.tree.command(name="insult", description="Curse someone with the slightest of inconviences")
        @app_commands.describe(user="The person you wish to insult")
        async def insult(interaction: discord.Interaction, user: discord.Member = None):
        ...
        @client.tree.command(name="8ball", description="Ask a question and have it answered by the magical 8ball")  # 8ball
        ...
        @client.tree.command(name="homo", description="How gay are you?")  # gay probability
        @app_commands.describe(person="The person you wish to target")
        async def homo(interaction: discord.Interaction, person: discord.Member = None):
            if person is None:
                person = interaction.user
        ...
        @client.tree.command(name="simp", description="Check how much of a SIMP you are")  # simp probability
        @app_commands.describe(person="The person you wish to target")
        async def simp(interaction: discord.Interaction, person: discord.Member = None):
            if person is None:
                person = interaction.user

            embed = discord.Embed(title="Simp machine", color=discord.Color.orange())
            embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/922598156842172508/923140639556775946/koffee4.png')
            embed.add_field(name=f"{person}", value="You are " + str(random.choices(range(1, 101))) + "% simp!")
            await interaction.response.send_message(embed=embed)

        @client.tree.command(name="stupid", description="Check how smart you really are")  # Stupid probability
        @app_commands.describe(person="The person you wish to target")
        async def dumb(interaction: discord.Interaction, person: discord.Member = None):
            if person is None:
                person = interaction.user

            embed = discord.Embed(title="Dumb calculator", color=discord.Color.orange())
            embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/922598156842172508/923140639556775946/koffee4.png')
            embed.add_field(name=f"{person}", value="You are " + str(random.choices(range(1, 101))) + "% stupid!")
            await interaction.response.send_message(embed=embed)

        @client.tree.command(name="laugh", description="KoffeeBot will respond accordingly")  # laugh command
        async def laugh(interaction: discord.Interaction):
            laughs = ['haha',
                      'lol',
                      'haha lol',
        ...
        @client.tree.command(name="o", description="Replace every vowel with the letter 'o'")
        @app_commands.describe(msg="Your message that will be transformed!")
        async def o(interaction: discord.Interaction, msg: str):
            msg = msg.lower()
            x = ''
            vowels = ['a', 'e', 'i', 'u']
        ...
        @client.tree.command(name="topic", description="Can't think of a conversation starter. Let us help!")
        async def topic(interaction: discord.Interaction):
            f = open("topics.txt", encoding="utf8").read().splitlines()
            rand_topic = random.choice(f)
            embed = discord.Embed(title="Topic generator", description=f"{rand_topic}", color=discord.Color.orange())
            await interaction.response.send_message(embed=embed)
